...
int main(int argc, const char* argv[])
{
   ...
   NSString *dayAfterTomorrowString = [dateFormat stringFromDate:dayAfterTomorrow];
	NSLog(@"Day after tomorrow's date is: %@", dayAfterTomorrowString);
   ...
   int weekday = [component weekday];
	if(weekday>5) //to check if we have passed thursday in the current week
	{
   ...
   NSString *earliestDateString = [dateFormat stringFromDate:earliestDate];
	NSLog(@"The earliest date among the given dates is: %@", earliestDateString);
   ...
   NSDictionary *days = [NSDictionary dictionaryWithObjectsAndKeys:
	             			  @"Sunday", [NSNumber numberWithInteger:1],
	             				...
	             				@"Thursday", [NSNumber numberWithInteger:5],
	...
	NSLog(@"First day of the month is %@,tenth day of the month is %@",[days objectForKey:firstDayOfMonth],[days objectForKey:tenthDayOfMonth]);
	[pool release];
	return 0;
}
